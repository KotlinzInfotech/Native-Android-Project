...
public abstract class CustomAsync<Params, Progress, Result> {
    ...
    static {
        ...
        THREAD_POOL_EXECUTOR = new ThreadPoolExecutor(10, 128, 1L, TimeUnit.SECONDS, CustomAsync.sPoolWorkQueue, CustomAsync.sThreadFactory);
        sHandler = new InternalHandler();
        CustomAsync.sDefaultExecutor = CustomAsync.THREAD_POOL_EXECUTOR;
    }
    ...
    public CustomAsync() {
        ...
        this.mWorker = (WorkerRunnable<Params, Result>) new WorkerRunnable<Params, Result>() {
            ...
            public Result call() throws Exception {
                CustomAsync.this.mTaskInvoked.set(true);
                Process.setThreadPriority(10);
                return (Result) CustomAsync.this.postResult(CustomAsync.this.doInBackground(this.mParams));
            }
        };
        this.mFuture = new FutureTask<Result>(this.mWorker) {
            ...
            protected void done() {
                try {
                    CustomAsync.this.postResultIfNotInvoked(((FutureTask<Result>) this).get());
                    return;
                } catch (Throwable t) {
    ...
    public static void execute(final Runnable runnable) {
        CustomAsync.sDefaultExecutor.execute(runnable);
    }
    ...
    public static void init() {
        CustomAsync.sHandler.getLooper();
    }

    private Result postResult(final Result result) {
        CustomAsync.sHandler.obtainMessage(1, (Object) new AsyncTaskResult(this, new Object[]{result})).sendToTarget();
        return result;
    }
    ...
    public static void setDefaultExecutor(final Executor sDefaultExecutor) {
        CustomAsync.sDefaultExecutor = sDefaultExecutor;
    }
    ...
    public final CustomAsync<Params, Progress, Result> execute(final Params... array) {
        return this.executeOnExecutor(CustomAsync.sDefaultExecutor, array);
    }

    public final CustomAsync<Params, Progress, Result> executeOnExecutor(Executor executor, Params... paramsArr) {
    ...
    protected final void publishProgress(final Progress... array) {
        if (!this.isCancelled()) {
            CustomAsync.sHandler.obtainMessage(2, (Object) new AsyncTaskResult(this, (Object[]) array)).sendToTarget();
        }
    }
    ...
    private static class AsyncTaskResult<Data> {
        ...
        AsyncTaskResult(final CustomAsync mTask, final Data... mData) {
            this.mTask = mTask;
            this.mData = mData;
        }
    }
