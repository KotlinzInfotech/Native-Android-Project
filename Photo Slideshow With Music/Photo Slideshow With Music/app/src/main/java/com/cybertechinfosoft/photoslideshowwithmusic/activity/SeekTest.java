...
public class SeekTest {
    ...
    public static boolean CanSeekAccurately(SharedPreferences prefs) {
        ...
        String str = ".mp3";
        String filename = "/sdcard/silence" + nextLong + str;
        ...
        try {
            ...
            try {
                ...
                for (i = 0; i < 80; i++) {
                    fileOutputStream.write(SILENCE_MP3_FRAME, 0,
                            SILENCE_MP3_FRAME.length);
                }
                MediaPlayer player = new MediaPlayer();
                player.setAudioStreamType(3);
                long start = (long) (SILENCE_MP3_FRAME.length * 70);
                long len = (long) (SILENCE_MP3_FRAME.length * 10);
                player.setDataSource(new FileInputStream(filename).getFD(),
                        start, len);
                player.prepare();
                before = 0;
                after = 0;
                ...
                player.start();
                i = 0;
                while (i < ItemTouchHelper.Callback.DEFAULT_DRAG_ANIMATION_DURATION
                        && before == 0) {
                    try {
